...
def main():
    ...
    train_queue = torch.utils.data.DataLoader(
        TrainDataset, batch_size=args.batch_size, pin_memory=True, num_workers=0, shuffle=True
    )

    test_queue = torch.utils.data.DataLoader(TestDataset, batch_size=1, pin_memory=True, num_workers=0, shuffle=True)
    ...
    for step in range(args.steps):
        for batch_idx, (input, _) in enumerate(train_queue):
            ...
            print(f"finetune-step:{step} loss:{loss.item()}")
